...
def upload_file_from_filename(bucket, filename, client, subdir = None):
    try:
        bucket = client.get_bucket(bucket)
        if subdir is not None:
            blob = bucket.blob(subdir+os.path.basename(filename))
        else:
...
def main_download_reports():
    bucket = 'ford_grs_biding'

    ## Load in all the initial configurations ##
    client = initiate_client(project = 'mp-adh-groupm-ca')
    exec_source("dev-google_get_auth_2.py")
    bidmanager_token = google_auth("miguel_dbm")
    config = json.loads(download_fromStorage(file_key = "google/FORD_DV_REPORT_PULLERS_config.json", bucket_name = "xax-configs", client = client, string = True))
    ...
    print('dbm_reports',dbm_reports)
    queryList = DBM_createQueryList(listQueries = dbm_reports)
    print('queryList',queryList)
    
    fnames_list = [] 
    print(range(len(config['report_puller']['dbm_report_ids'])))
    for i in range(len(config['report_puller']['dbm_report_ids'])):
        ...
        token = json.loads(bidmanager_token['body'])['access_token'])
        reportStartTimeStamp = dbm_reports['queries'][queryList.index(config['report_puller']['dbm_report_ids'][i])]['schedule']['startTimeMs']
        reportEndTimeStamp = dbm_reports['queries'][queryList.index(config['report_puller']['dbm_report_ids'][i])]['schedule']['endTimeMs']
        print('reportStartTime',reportStartTimeStamp,'reportEndTime',reportEndTimeStamp)
        print('type',type(reportStartTimeStamp))
        print(int(reportStartTimeStamp))
        reportStartTime = datetime.fromtimestamp(int(reportStartTimeStamp)/1000).strftime("%Y-%m-%d")
        reportEndTime = datetime.fromtimestamp(int(reportEndTimeStamp)/1000).strftime("%Y-%m-%d")
        print(reportStartTime,reportEndTime)

        f_name = 'C:/tmp/'+reportStartTime+'-'+reportEndTime+'-'+config['report_puller']['dbm_report_names'][i]+'.csv'
