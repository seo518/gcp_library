...
class TrendReq(object):
    ...
    def interest_by_region(self, resolution='COUNTRY', inc_low_vol=False,
        ...
        elif self.geo == 'CA' and resolution in ['DMA', 'CITY', 'REGION']:
            self.interest_by_region_widget['request'][
                'resolution'] = resolution
...
# range_time = days
def getTime_Seq(date_now, range_time):
    date_now = date.today()
    date_now = date_now - timedelta(days=range_time)
    return(date_now)

# f = string = interest_over_time | [get_historical_interest] | [interest_by_region] | related_topics | related_queries | top_charts | suggestions | categories
# time_range = integer = 30, these are in days
def getPyTrends(f, time_range, keywords, pytrends, geo = 'CA'):
    
    # To implement the other functions in f
    
    if f is "get_historical_interest":
        pytrends.build_payload(kw_list = keywords , cat=0, timeframe='today '+str(math.ceil(time_range/30))+'-m', geo='CA', gprop='')
        x = pytrends.get_historical_interest(keywords,
                                         year_start=int(d.strftime("%Y")),
                                         month_start=int(d.strftime("%m")),
                                         day_start=int(d.strftime("%d")),
                                         hour_start=0,
                                         year_end=int(date.today().strftime("%Y")),
                                         month_end=int(date.today().strftime("%m")),
                                         day_end=int(date.today().strftime("%d")),
                                         hour_end=0,
                                         cat=0,
                                         geo='CA',
                                         gprop='',
                                         sleep=3)
        return x
    if f is "interest_by_region":
        x = pytrends.build_payload(kw_list = keywords, cat=0, timeframe='today 1-m', geo='CA', gprop='')
        x = pytrends.interest_by_region(resolution='CITY', inc_low_vol=True, inc_geo_code=False)
        return x
...
def main():
    ...
    w_list = json.loads(download_fromStorage(file_key="google/trends_world_list_config.json", bucket_name='xax-configs', client=client,
                                string=True))
    
    d = getTime_Seq(date_now = date.today(), range_time = 30)
    
    #put time range in config file
    for n in range(len(w_list['word_list'])):
        file = getPyTrends(f = "interest_by_region", keywords = [w_list['word_list'][n]], time_range = 30, pytrends = pytrends)
        file_name = '/tmp/'+date.today().strftime("%Y-%m-%d %H%M%S")+"-"+w_list['word_list'][n]+'-interest_by_region.csv'
        #file_name = 'C:/tmp/'+date.today().strftime("%Y-%m-%d")+"-"+w_list['word_list'][n]+'-interest_by_region.csv'
        
        #file.to_csv("C:/tmp/tes2t.csv", encoding = 'utf-8')
        file.to_csv(file_name, encoding ='utf-8')
        
        
        upload_file_from_filename(bucket = 'xax-trends', filename = file_name, client = client)
        time.sleep(1)
