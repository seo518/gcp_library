...
def main():
    ...
    input_text = download_fromStorage(file_key = inputs_key, bucket_name = "xax-input-mapsapi", client = client, string = True)
    ...
    for n in range(len(input_list)):
        ...
        geocode_result = gmaps.geocode(input_list[n])
        print(geocode_result)
        
        if len(geocode_result) == 0:
            gmaps = googlemaps.Client(key=api_key)
            geocode_result = gmaps.places(input_list[n])
            print(geocode_result)
