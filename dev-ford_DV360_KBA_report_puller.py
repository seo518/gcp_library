...
def main():
    ...
    bidmanager_token = google_auth("bidmanager")
    ...
    for i in range(len(config['report_puller']['dbm_report_ids'])):   
        ...
        token = json.loads(bidmanager_token['body'])['access_token'])
        reportStartTimeStamp = dbm_reports['queries'][queryList.index(config['report_puller']['dbm_report_ids'][i])]['reportDataStartTimeMs']
        reportEndTimeStamp = dbm_reports['queries'][queryList.index(config['report_puller']['dbm_report_ids'][i])]['reportDataEndTimeMs']
        ...
        f_name = '/tmp/'+reportStartTime+'-'+reportEndTime+'-'+config['report_puller']['dbm_report_names'][i]+'.csv'
        # f_name = 'C:/tmp/'+datetime.datetime.today().strftime('%Y-%m-%d')+'-'+config['report_puller']['dbm_report_names'][i]+'.csv'
        
        f = open(f_name, 'wb')
        f.write(csv_write.data)
        f.close()        
        upload_file_from_filename(bucket ='ford-grs-kba-report', filename = f_name, client = client)
