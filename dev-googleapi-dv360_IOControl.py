...
working_dir = "C:/Users/miguel_ajon/Documents/gcp_library/googleapi python/APIJson"
...
def deleteTargetingOption(advertiser_id, lineItem_id, targeting_type, targetingOptionId, api_url, token, negative = False):
    api_url = re.sub("{ADVERTISING_ID}",str(advertiser_id), api_url)
    api_url = re.sub("{LINE_ITEM_ID}",str(lineItem_id), api_url)
    api_url = re.sub("{TARGETING_TYPE}",str(targeting_type), api_url)
    api_url = re.sub("{ASSIGNED_TARGETING_OPTION_ID}",str(targetingOptionId), api_url)
    
    x = request_delete(url = api_url,  headers = {"Authorization": "Bearer "+ token})
    x = json.loads(x.data)
    return(x)
...
def advertiser_insertionOrders_list(advertiser_id, api_url, token):
    api_url = re.sub("{ADVERTISING_ID}",str(advertiser_id), api_url)
    x = request_get(url = api_url, headers = {"Authorization": "Bearer "+ token})
    x = json.loads(x.data)
    return(x)
